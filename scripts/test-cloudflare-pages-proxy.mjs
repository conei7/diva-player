...
try {
  ...
  assert.equal(namedResponse.headers.get('x-diva-origin-role'), 'primary');
  assert.equal(namedResponse.headers.get('x-diva-standby-state'), 'missing');
  ...
  assert.deepEqual(deletedKeys, ['quick_tunnel_standby_url']);
  ...
  const standbyProof = createHmac('sha256', proofKey)
    .update(`${timestamp}\nstandby\n${standbyUrl}`)
    .digest('hex');
  const writesBeforeStandby = writes.length;
  const deletesBeforeStandby = deletedKeys.length;
  const fetchesBeforeStandby = calls.length;
  ...
  assert.equal(standbyUpdated.status, 410);
  assert.equal(writes.length, writesBeforeStandby);
  assert.equal(deletedKeys.length, deletesBeforeStandby);
  assert.equal(calls.length, fetchesBeforeStandby);

  const primaryOnlyCalls = [];
  const requestedTunnelKeys = [];
  globalThis.fetch = async (target, init) => {
    primaryOnlyCalls.push({ target: String(target), init });
  ...
  const primaryOnlyEnv = {
    PAGES_PROXY_KEY: 'test-proxy-key',
    TUNNEL_CONFIG: {
      getWithMetadata: async key => {
        requestedTunnelKeys.push(key);
        return {
          value: ({
            quick_tunnel_primary_url: tunnelUrl,
            quick_tunnel_url: 'https://legacy-primary.trycloudflare.com',
  ...
  const primaryUnavailableResponse = await proxyBackend({
    request: new Request('https://diva-player.pages.dev/backend-api/api/health?full=1'),
    env: primaryOnlyEnv,
  });
  assert.equal(primaryUnavailableResponse.status, 503);
  assert.equal(await primaryUnavailableResponse.text(), 'primary failed');
  assert.equal(primaryOnlyCalls.length, 1);
  assert.equal(
    primaryOnlyCalls[0].target,
    `${tunnelUrl}/backend-api/api/health?full=1`,
  );
  assert.deepEqual(
    requestedTunnelKeys.sort(),
    ['quick_tunnel_primary_url', 'quick_tunnel_url'],
  );
  assert.equal(primaryUnavailableResponse.headers.get('x-diva-origin-role'), 'primary');
  assert.equal(primaryUnavailableResponse.headers.get('x-diva-standby-state'), 'missing');
  ...
  const postResponse = await proxyBackend({
    request: new Request(
  ...
  assert.deepEqual(postCalls.map(call => call.body), [postBody]);
  assert.equal(postResponse.headers.get('x-diva-origin-role'), 'primary');
  assert.equal(postResponse.headers.get('x-diva-standby-state'), 'missing');
...
try {
  ...
  assert.match(syncResult.stdout, /"originRole":"primary"/);
  ...
  const retiredStandbyResult = spawnSync(
    'sh',
    [fixturePath(join(scriptsDirectory, 'sync-quick-tunnel-to-cloudflare.sh'))],
    {
      cwd: projectDirectory,
      encoding: 'utf8',
      env: { ...syncEnvironment, DIVA_TUNNEL_ORIGIN_ROLE: 'standby' },
    },
  );
  assert.equal(retiredStandbyResult.status, 78);
  assert.match(retiredStandbyResult.stderr, /standby Quick Tunnel origin is retired/);
  assert.doesNotMatch(
    retiredStandbyResult.stdout + retiredStandbyResult.stderr,
    /test-sync-token|test-origin-proof-key/,
  );
} finally {

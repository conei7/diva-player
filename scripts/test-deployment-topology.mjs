...
assert.match(sbcTrivyInstaller, /stat -c '%u:%g:%a:%h'/u);
assert.match(sbcTrivyInstaller, /0:0:555:1/u);
assert.match(workflow, /python -B scripts\/test-attest-disaster-backup-payloads\.py/u);
assert.match(workflow, /npm run test:sbc-trivy-installer/u);
...
assert.match(databaseRoleContractStep, /grep -qx '27\|27\|0'/);

      ...
      <div className="flex flex-wrap items-center gap-2">
        <div className="relative min-w-[13rem] flex-1 lg:max-w-sm">
          ...
          <input
            ...
            placeholder={t('曲名・アーティストを検索')}
            className="search-input min-h-10 w-full rounded-xl pl-10 pr-10 text-sm"
          />
          {query && (
            <button type="button" onClick={() => onQueryChange('')} className="absolute right-1.5 top-1/2 flex h-8 w-8 -translate-y-1/2 items-center justify-center rounded-lg text-neutral-500 hover:bg-white/10 hover:text-white" aria-label={t('曲検索をクリア')}>
        ...
        <div className="hidden items-center rounded-xl bg-black/20 p-1 sm:flex" aria-label={t('曲の並べ替え')}>
        ...
        >
          ...
          {t('健全性')}{healthIssueCount > 0 ? ` ${healthIssueCount}` : ''}
        </button>
        ...
        {duplicateCount > 0 && (
          <button type="button" disabled={externalLinked} onClick={onRemoveDuplicates} className="min-h-10 rounded-xl border border-amber-300/25 px-3 text-xs text-amber-200 transition-colors hover:bg-amber-300/10 disabled:opacity-40">
            {t('重複削除 {count}', { count: duplicateCount })}
          </button>
        )}
        ...
        >
          ...
          {selectionMode ? t('選択を終了') : t('複数選択')}
        </button>
        {selectionMode && (
          <button type="button" onClick={onSelectAll} className="min-h-10 rounded-xl border border-white/10 px-3 text-xs text-neutral-300 transition-colors hover:bg-white/[0.06] hover:text-white">{t('すべて選択')}</button>
        )}
      </div>

      <div className="mt-2 flex items-center justify-between px-1 text-[10px] text-neutral-600">
        <span>{query ? t('{result} / {total}曲を表示', { result: resultCount, total: totalCount }) : t('{count}曲', { count: totalCount })}</span>
        {externalLinked && <span>{t('外部同期の順序を保持しています')}</span>}
      </div>
    </div>
  );
}

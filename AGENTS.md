...
- ユーザーが「実装して」「修正して」「対応して」「進めて」と依頼した場合は、調査や計画だけで止めず、実装、妥当なテスト、lint、production buildまで進める。
- 実装・修正後は、利用者向け挙動、恒久仕様、運用、進捗のうち変更された内容を、下記の文書更新ルールに従って既存文書へ記録する。
- ユーザーが「次に何ができる」「計画して」「実装手順を示して」と依頼した場合は、コードや文書を変更せず、現状を読み取り確認したうえで、対象ファイル、実装順、データ構造・API・UIの変更、互換性、テスト、配備、注意点を含む詳細な実装手順をチャットで提示する。
- 計画だけを求められた場合は、ユーザーが続けて実装を依頼するまでcommit、push、配備を行わず、計画用Markdownも作成しない。
